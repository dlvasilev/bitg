...
// (Original One) =>  Create a temporary variable, initialise it with zero and using a
// for loop iterate over the array returning the final value of the variable.
// There are some problems though: For something as simple as a sum of an array,
// 7 lines of code seem quite a lot.
const sumOne = (array) => {
    let result = 0;
...
// (Reduced One) => The solution that remedies both those problems is to use the reduce function
// (in other languages also called fold or aggregate). 
// In a single expression we iterate over each of the array elements adding them together 
// (stating the sum’s default and initial value is zero)
...
// (Original One) => In order to not destroy the input array, 
// we must clone it first and then transform the values in the new array. 
// This can easily be forgotten introducing a potentially unwanted side effect in the application.
...
// (Reduced One) => The cloning problem can be avoided altogether using the map function. 
// For a given array it returns a new array where each element is the corresponding element 
// in the original array transformed using the provided function
 ...
 */
// (Original One) => The first naïve solution suffers from the problem that it pushes
// a new element to an array every iteration. This expands the array and may
// cause it to reallocate in memory being slow
const squaresBad = (n) => {
    let result = [];
    for (let i = 1; i <= n; i += 1) {
        result.push(i * i);
...
// (Original One) => The second approach instantiates the array of correct size beforehand
// avoiding this problem, but we can easily make a mistake when assigning the current value result[i - 1]. 
const squaresOne = (n) => {
...
const squaresTwo = (n) => Array.apply(null, Array(n)).map((_, n) => (n + 1) * (n + 1));
